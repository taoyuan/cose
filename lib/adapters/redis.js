...
exports.store = function (settings) {
    ...
    if (express3 && !cr1) {
        console.warn('connect-redis version is too high, you should using <= 1.4.7 for express version 3.x');
        process.exit(1);
    }

    if (!express3 && cr1) {
        console.warn('connect-redis version is too low, you should using >= 2.0.0 for express version 4.x');
        process.exit(1);
    }
